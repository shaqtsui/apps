...
it contains 2 parts:
  kernels:
    IPython - Python programe
    IJulia - Julia programe
    IRkernel - R programe
...
REPL
  a program implemented with julia language

  internal repl
    key press -> starter process(e.g. bash, emacs) -via stio, instantly send-> repl


  3rd part repl
    key press -> starter process(e.g. bash, emacs) -> interface program(e.g. jupyter-console, jupyter-qtconsole, jupyter-notebook, emacs-jupyter) -via ZeroMQ, buffered & send-> repl
