...
Prime + Recursion
  ...
  If all addition are the same, it's Linear Recursion
  Cognition down to Recursion, then it's Thorough

  instance:
    Fa Zhi Jie
    Learning/Study/Cognition
    ...
    Function
      ...
      Total Linear: Identity -> Identity, Generator -> Generator(Change Pairs are same)
      Partial Linear: Generator -> Generator(Change Pairs are same)
      Isomorphism is Total Linear
      deferential:
...
lineinfile
'target line' have location & content, identified by regexp or line.
if state == absent
  target line = nil
else
  location = location || insertxxxxx
  content = 'new content'

|                                            | replace | insert                                                        | delete |
| target file location(path)                 | M       | M                                                             | M      |
| target line location(regexp)               | M       | target line location not available(not specified or no match) | M      |
| insert location(insertafter, insertbefore) |         | M                                                             |        |
| new content                                | M       | M                                                             |        |
...
~/.julia/compiled - compiled julia bytes
...
*** WTGA
Process
| Onboard Firmeware | UEFI(clover)    | macOS 10.15 |
|                   |                 | macOS 10.14 |
|                   |                 | ...         |
|                   | UEFI(microsoft) | windows 10  |
|                   |                 | windows 8.1 |
|                   |                 | windows pe  |
|                   |                 | ...         |
|                   | UEFI(deepin)    | ubuntu      |
...
A OS in BIOS + ESP(EFI system partion)
ESP need disk in GPT/GUID partion
ESP have a special GUID
ESP have file system: fat32/fat16
ESP part:
microsoft(windows), clover(mac), deepin(linux)
...
*** homebrew vs apt vs yum
|          | pkg scope                                  | support pkg versions | impl languae |
| homebrew | user local(in /usr/local, so no need sudo) | single               | ruby         |
| apt, yum | OS(overwrite OS pkg, so need sudo)         | multiple             | c?           |
